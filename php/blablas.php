<?php
	...
	*/
	...
	pb_html_debut("Cuiteur | Bablas", CHARSET, "../styles/cuiteur.css");
	
		echo '<body>',
			'<div id="bcPage"><header>';
		
			pb_aff_cuiteur_Header_Liens();
			pb_aff_cuiteur_Header_Titre("Les blablas de ".$utilisateur1['usPseudo']);
			...
			$Req="SELECT usPseudo, usID, usNom, blDate, blID, blHeure, blTexte, blAvecCible, blIDOriginal, usAvecPhoto
				FROM blablas, users
				WHERE blIDAuteur=usID
				AND usID='".mysqli_real_escape_string($BD, ($utID))."'
			...
			{
				echo	'<li id="bcPlus">'.
				'<a href="blablas.php?foisBlablas='.($_GET['foisBlablas']+1).'&utPseudo='.($utilisateur1['usPseudo']).'"><strong>Plus de blablas</strong></a>';
			}
			echo '</li>',
				'</ul>',
				'</section>';
